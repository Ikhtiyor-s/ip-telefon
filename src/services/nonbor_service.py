...
class NonborService:
    ...
    async def get_order_full_data(self, order_id: int) -> Dict:
        ...
        if user:
            ...
            # user.lang - bu mijozning tili, seller_language ni O'ZGARTIRMAYMIZ
            # seller_language faqat business ob'ektidan olinadi (yuqorida)
    ...
    async def update_business_group(self, biz_id: int, group_id: str) -> bool:
        ...
        timeout = aiohttp.ClientTimeout(total=10)
        try:
            async with session.put(url, json={"group_id": group_id, "_source": "autodialer"}, timeout=timeout) as resp:
